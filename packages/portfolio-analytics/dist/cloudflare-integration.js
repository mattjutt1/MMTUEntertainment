...
class CloudflareIntegration {
     ...
     */
    async getProductMetrics(product, timeRange = '24h') {
     ...
     */
    async getPortfolioMetrics(products, timeRange = '24h') {
        const productMetrics = await Promise.all(products.map(product => this.getProductMetrics(product, timeRange)));
        return this.aggregateMetrics(productMetrics);
    }
    /**
     * Query uptime for a specific product
     */
    async getUptimeMetrics(product, timeRange = '24h') {
     ...
     */
    async queryWorkerObservability(query) {
    ...
    convertTimeRangeToTimeframe(timeRange) {
        const now = new Date();
        const from = new Date();
        switch (timeRange) {
            case '1h':
    ...
    parseCloudflareMetrics(requestsData, performanceData, errorData, bandwidthData) {
