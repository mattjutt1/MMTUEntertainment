...
exports.PortfolioDashboardDataProvider = void 0;
const dora_metrics_1 = require("./dora-metrics");
const cloudflare_integration_1 = require("./cloudflare-integration");
const posthog_portfolio_1 = require("./posthog-portfolio");
class PortfolioDashboardDataProvider {
    constructor(config) {
        this.config = config;
        this.doraCollector = new dora_metrics_1.DORAMetricsCollector(config.github.token, config.products);
        this.cloudflareIntegration = new cloudflare_integration_1.CloudflareIntegration(config.cloudflare.accountId, config.cloudflare.apiToken);
        this.posthogAnalytics = new posthog_portfolio_1.PostHogPortfolioAnalytics(config);
    }
    /**
     * Get comprehensive portfolio dashboard data
     */
    async getPortfolioDashboard(timeRange = '24h') {
        try {
            ...
            const productMetrics = await Promise.all(this.config.products.map(async (product) => {
    ...
    async getRealtimeStatus() {
        return Promise.all(this.config.products.map(async (product) => {
            try {
                ...
                const status = this.determineRealtimeStatus(uptimeMetrics.percentage, cloudflareMetrics.performance.averageResponseTime, cloudflareMetrics.errors.rate, product.alertingConfig);
                return {
     ...
     */
    async getDORATrends(timeRange = '30d') {
    ...
    async getRecentDeployments(product, limit = 5) {
        try {
    ...
    async getActiveIncidents(product) {
        try {
    ...
    calculateHealthStatus(cloudflareMetrics, activeIncidents) {
    ...
    calculateOverallHealth(productMetrics) {
    ...
    async generateAlerts(productMetrics) {
        const alerts = [];
    ...
    determineRealtimeStatus(uptime, responseTime, errorRate, alertingConfig) {
        if (uptime < (alertingConfig.uptimeThreshold - 1) ||
            errorRate > (alertingConfig.errorRateThreshold * 2) ||
            responseTime > (alertingConfig.responseTimeThreshold * 2)) {
            return 'critical';
        }
        if (uptime < alertingConfig.uptimeThreshold ||
...
exports.PortfolioDashboardDataProvider = PortfolioDashboardDataProvider;
