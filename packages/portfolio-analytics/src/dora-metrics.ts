...
export class DORAMetricsCollector {
  ...
  private products: PortfolioProduct[];
   ...
   */
  async calculateDORAMetrics(timeRange: TimeRange = '30d'): Promise<DORAMetrics> {
   ...
   */
  private async getDeploymentsData(timeRange: TimeRange): Promise<DeploymentData[]> {
    ...
    for (const product of this.products) {
      try {
        ...
        const deploymentRuns = workflowRuns.workflow_runs.filter(run => 
   ...
   */
  private async getIncidentsData(timeRange: TimeRange): Promise<IncidentData[]> {
   ...
   */
  private calculateDeploymentFrequency(deployments: DeploymentData[], timeRange: TimeRange) {
    ...
    return {
      ...
      trend: this.calculateTrend(successfulDeployments, 'deployment') as 'increasing' | 'stable' | 'decreasing'
    };
  }
   ...
   */
  private calculateLeadTimeForChanges(deployments: DeploymentData[]) {
    const productionDeployments = deployments.filter(d => 
      d.environment === 'production' && d.duration !== undefined
    );
    ...
    return {
      ...
      trend: this.calculateTrend(productionDeployments, 'leadtime') as 'improving' | 'stable' | 'degrading'
    };
  }
   ...
   */
  private calculateMTTR(incidents: IncidentData[]) {
    ...
    return {
      ...
      trend: this.calculateTrend(resolvedIncidents, 'recovery') as 'improving' | 'stable' | 'degrading'
    };
  }
   ...
   */
  private calculateChangeFailureRate(deployments: DeploymentData[], incidents: IncidentData[]) {
    ...
    return {
      ...
      trend: this.calculateTrend(failedDeployments, 'failure') as 'improving' | 'stable' | 'degrading'
    };
  }

  // Helper methods
  private getTimeRangeDate(timeRange: TimeRange): Date {
    const now = new Date();
    switch (timeRange) {
  ...
  private extractEnvironmentFromRun(run: any): string {
    const name = run.name?.toLowerCase() || '';
    const path = run.path?.toLowerCase() || '';
    
    if (name.includes('prod') || path.includes('prod')) return 'production';
    if (name.includes('staging') || path.includes('staging')) return 'staging';
    return 'development';
  }

  private mapWorkflowStatusToDeploymentStatus(conclusion: string | null): DeploymentData['status'] {
    switch (conclusion) {
  ...
  private extractPRNumber(run: any): number | undefined {
  ...
  private extractSeverityFromIssue(issue: any): IncidentData['severity'] {
    const labels = issue.labels?.map((l: any) => l.name?.toLowerCase()) || [];
    
    if (labels.includes('critical') || labels.includes('p1')) return 'critical';
    if (labels.includes('high') || labels.includes('p2')) return 'high';
    if (labels.includes('medium') || labels.includes('p3')) return 'medium';
    return 'low';
  }

  private extractRecoveryActions(body: string): string[] {
    // Simple extraction of recovery actions from issue body
    const lines = body.split('\n');
    const recoverySection = lines.findIndex(line => 
      line.toLowerCase().includes('recovery') || 
      line.toLowerCase().includes('resolution') ||
      line.toLowerCase().includes('fix')
    );
    
    if (recoverySection === -1) return [];
  ...
  private calculateTrend(data: any[], type: string): string {
